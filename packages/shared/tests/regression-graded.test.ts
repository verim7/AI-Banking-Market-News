...
/**
 * The classifier, measured against 80 articles a reader graded by hand.
 *
...
const articles = readJsonl<Pending>('data/review/pending.jsonl');
const grades = new Map(
  readJsonl<Decision>('data/review/decisions/2026-08-25-1.jsonl')
...
describe('the rules against 80 hand-graded articles', () => {
  it('has the whole graded set available', () => {
    expect(scored).toHaveLength(80);
  ...
  it('reports the overall agreement, so a regression is visible as a number', () => {
    ...
    console.log(`  above MIN_AI_INTENSITY:      `
      + `${scored.filter((s) => s.c.aiIntensity >= MIN_AI_INTENSITY).length}/80`);
    ...
    //
    // Before this pass the same measurement read 6/25 and 2/15: the rules
    // called six pieces of commentary deployments and recognised two of the
    // fifteen real ones. Both numbers moved at once, which is the only kind of
    // improvement worth having here — precision alone is free if you classify
    // nothing as a deployment.
    expect(dAsDeployment).toBeLessThanOrEqual(1);
    expect(aAsDeployment).toBeGreaterThanOrEqual(13);
  });
});
