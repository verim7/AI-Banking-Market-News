...
| **Agentic Swiss Banks** | The same view, narrowed to named Swiss institutions — not the region tag, which is wrong in both directions. See [docs/swiss-coverage.md](docs/swiss-coverage.md) |
...
| **Check sources** | Fetches every source, reports its yield, and regenerates the source ranking | **Weekly, Monday 07:40 UTC**, and on demand |
...
**Which sources are worth having** is answered in
[`docs/source-quality.md`](docs/source-quality.md), regenerated by Check sources
from everything in the database. It ranks each source by evidence of production
first, then mean AI focus, then whether the use case is described, with volume
dampened so a firehose cannot outrank a specialist — and it names sources with
enough articles and no deployments at all as candidates to drop. Read it before
adding or removing a source; the patterns it records are what should drive the
next one.
